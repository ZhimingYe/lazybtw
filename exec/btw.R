...
#| launcher:
#|   default-packages: [base, datasets, utils, stats, methods, lazybtw]
library(utils)
...
if (version) {
  cat(format(utils::packageVersion("lazybtw")), "\n")
  quit(status = 0)
}
...
btw_docs_help <- function(topic, package) {
  if (grepl("::", topic, fixed = TRUE)) {
    ...
    btw_output(btw_this(lazybtw:::as_btw_docs_topic(parts[1], parts[2])))
  } else if (grepl("^\\{.+\\}$", topic)) {
    pkg_name <- sub("^\\{(.+)\\}$", "\\1", topic)
    btw_output(btw_this(lazybtw:::as_btw_docs_package(pkg_name)))
  } else if (has_value(package)) {
    btw_output(btw_this(lazybtw:::as_btw_docs_topic(package, topic)))
  } else {
    result <- tryCatch(
      btw_this(lazybtw:::as_btw_docs_topic(NULL, topic)),
      error = function(e) NULL
    )
    if (is.null(result)) {
      btw_output(btw_this(lazybtw:::as_btw_docs_package(topic)))
    } else {
      btw_output(result)
    }
  }
...
btw_docs_news <- function(package, search) {
  ...
  btw_output(lazybtw:::btw_tool_docs_package_news_impl(package, search_term))
}

btw_pkg_document <- function(path) {
  btw_output(lazybtw:::btw_tool_pkg_document_impl(path))
}

btw_pkg_check <- function(path) {
  btw_output(lazybtw:::btw_tool_pkg_check_impl(path))
}

btw_pkg_test <- function(path, filter) {
  btw_output(
    lazybtw:::btw_tool_pkg_test_impl(
      path,
...
btw_pkg_load <- function(path) {
  btw_output(lazybtw:::btw_tool_pkg_load_all_impl(path))
}

btw_pkg_coverage <- function(path, file, json = FALSE) {
  result <- lazybtw:::btw_tool_pkg_coverage_impl(
    path,
...
btw_info_platform <- function(json = FALSE) {
  result <- lazybtw:::btw_tool_sessioninfo_platform_impl()
  if (json) {
  ...
  if (check && length(pkgs) > 0) {
    if (json) {
      results <- lapply(pkgs, function(pkg) {
        result <- lazybtw:::btw_tool_sessioninfo_is_package_installed_impl(pkg)
        S7::prop(result, "extra")
      })
      btw_json_output(results)
    } else {
      for (pkg in pkgs) {
        btw_output(lazybtw:::btw_tool_sessioninfo_is_package_installed_impl(pkg))
      }
    }
  } else {
    if (length(pkgs) == 0) {
    ...
    result <- lazybtw:::btw_tool_sessioninfo_package_impl(pkgs, deps_val)
    if (json) {
